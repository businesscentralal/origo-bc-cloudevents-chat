...
- **Any OpenAI-compatible endpoint** — Works with Ollama, vLLM, LM Studio, LocalAI, text-generation-webui, or any service that exposes `/v1/chat/completions`.
...
- **Configurable model** — Default model set in Cloud Events Setup with a live lookup against your endpoint. Per-role model override on the Chat Role card.
- **Configurable limits** — API timeout (30–600 seconds) and max output tokens adjustable from setup without code changes.
- **Permission gate** — Service key access and setup fields require the `CE LLM Svc` permission set. Server-side message types (`LLM.Model.Call`, `LLM.Model.List`) are gated the same way.
...
Users open the MCP Chat panel (FactBox or Focus page) on any BC page. The chat control add-in connects to the LLM through the in-process tool server — no external proxy or Azure Function needed. The model receives a system prompt with the user's identity, company context, memory, and an API primer. When the user asks a question, the model calls tools to fetch live data, then responds in natural language.
...
- The shared service API key is stored in module-scoped IsolatedStorage, accessible only to users with the `CE LLM Svc` permission set.
